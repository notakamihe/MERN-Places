...
router.get('/', async (req, res) => {
    const tags = await Tag.find()//.map(async t => {
    //     var tObject = t.toObject()

    //     tObject['popularity'] = await utils.getPopularity(t._id)
    //     res.json(tObject)
    // })
    
    const tagsWPop = await Promise.all(await tags.map(async t => {
        var tObject = t.toObject()
...
router.get('/:id', async (req, res) => {
    Tag.findById(req.params.id).then(async result => {
        ...
        resultObj['popularity'] = await utils.getPopularity(req.params.id)
        res.json(resultObj)
    }).catch(err => {
        res.status(400).json({

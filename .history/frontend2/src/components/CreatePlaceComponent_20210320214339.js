...
export default function CreatePlaceComponent() {
    ...
    const handleChangeInput = (index, event) => {
        ...
        setInputFields(newAdditionalFields);
      }
    
    const handleAddFields = () => {
        setInputFields([...additionalFields, { field: '', value: '' }])
    }
    
    const handleRemoveFields = idx => {
        setInputFields(additionalFields.slice(0, idx).concat(additionalFields.slice(idx+1, additionalFields.length)));
    }

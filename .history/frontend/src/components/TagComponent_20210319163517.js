...
export default function TagComponent(props) {
    ...
    useEffect(() => {
        loadTagInfo(props.id)
    }, [])

    const loadTagInfo = (id) => {
        axios.get(axios.defaults.baseURL + `api/tags/${id}`).then(res => {
            setTag(res.data)
            return res.data
        }).catch(err => {
        ...
        <div>
            {
                ...
                <div>
                    ...
                    <div className="d-flex px-5 mt-5" style={{flexWrap: "wrap"}}>
                            ...
                            <div className="d-flex" style={{flexWrap: 'wrap', justifyContent: 'space-evenly'}}>
                                {
                                    places.map(p => (
                                        <Link 
                                            key={p._id}
                                            className="rounded" 
                                            style={{width: 250, marginBottom: 64, color: "#f50057"}}
                                            to={`/places/${p._id}`}
                                        >
                                            <div 
                                                style={{
                        ...
                        <div className="d-flex mt-4" style={{flex: 1, flexWrap: 'wrap', alignItems: 'baseline', height: 'fit-content'}}>
                            {
